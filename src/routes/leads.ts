...
router.get("/custom_fields", async function (req, res) {
  try {
    ...
    const { data: leadsCustomFields } = await amoCrmAxios.get('/leads/custom_fields');
    const { data: contactsCustomFields } = await amoCrmAxios.get('/contacts/custom_fields');
    const { data: companiesCustomFields } = await amoCrmAxios.get('/companies/custom_fields');
    res.json({
      leadsCustomFields,
